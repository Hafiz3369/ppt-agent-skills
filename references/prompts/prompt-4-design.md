...
#### 消除盒子感的 5 种具体手段（必须至少用 1 种/页）

```css
/* 1. 负 margin 侵入：卡片突破 Grid gap 产生叠压 */
.card-invade { margin-top: -24px; position: relative; z-index: 3; }

/* 2. 出血定位：元素突破安全区延伸到画布边缘 */
.bleed-left { position: absolute; left: -40px; width: calc(100% + 80px); }

/* 3. 跨区域装饰线：渐隐线穿过多张卡片的间隙 */
.cross-divider {
  position: absolute; left: 5%; right: 5%; height: 1px;
  background: linear-gradient(90deg, transparent, var(--accent-1), transparent);
  opacity: 0.15; z-index: 5;
}

/* 4. 背景色融合：相邻卡片共享背景色，仅用极细线分隔 */
.card-merged { background: transparent; border-right: 1px solid var(--card-border); }

/* 5. 装饰元素跨卡片：一个圆/线条横跨两张卡片的边界 */
.deco-cross {
  position: absolute; top: 50%; left: calc(50% - 24px);
  width: 48px; height: 48px; border-radius: 50%;
  border: 2px solid var(--accent-1); opacity: 0.3;
  z-index: 5; /* 浮在卡片之上 */
}
```
...
### 硬底线（仅 4 条 -- 违反会导致跨页崩溃的底层约束）

| # | 硬底线 | 原因 |
|---|--------|------|
| 1 | **画布 1280x720px，body overflow:hidden** | 超出画布的内容无法显示 |
| 2 | **font-family 统一为** `'PingFang SC', 'Microsoft YaHei', system-ui, sans-serif` | 跨页字体不一致会让翻页时视觉"跳戏" |
| 3 | **CSS 变量必须从 {{STYLE_DEFINITION}} 复制**，卡片/文字/背景颜色优先通过 var(--xxx) 引用 | 风格统一的硬保证 |
| 4 | **每张卡片 overflow:hidden** | 防止内容溢出到相邻区域 |

> **硬底线之外的一切 CSS 手段完全自由。** 没有任何 CSS 属性是"禁止使用"的。text-align:center、justify-content:center、任意 border-radius 值、任意 padding 值——当设计意图需要时，都是合法的工具。

### CSS 能力释放（鼓励而非强制）

你拥有浏览器全部 CSS 能力。以下是我们鼓励你大胆使用的高级技法——它们能把普通的 HTML 页面提升到"发布会级"视觉品质：
...
- `@keyframes` + `animation` → 呼吸脉冲、渐入效果（HTML 预览加分）
- `writing-mode: vertical-rl` → 侧栏装饰文字
- `filter: blur() / brightness()` → 背景模糊、亮度调节
