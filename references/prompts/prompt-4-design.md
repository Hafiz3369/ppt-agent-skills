...
## ★★★ 反面教材 vs 正面教材（强制参考 -- 看完再写代码） ★★★

> **LLM 最常犯的错误**：用"网页前端安全排版"思维写 PPT。以下是同一内容的两种实现，你必须走右边那条路。

### 场景 1：数据页的核心数字

❌ **网页思维**（等大卡片、均匀字号、安全 padding）：
```html
<div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:20px; padding:40px;">
  <div style="background:#1E1E1E; padding:24px; border-radius:12px; text-align:center;">
    <div style="font-size:24px; color:#F5C518;">30,000+</div>
    <div style="font-size:14px; color:rgba(255,255,255,0.6);">注册用户</div>
  </div>
  <div style="background:#1E1E1E; padding:24px; border-radius:12px; text-align:center;">
    <div style="font-size:24px; color:#F5C518;">7,000+</div>
    <div style="font-size:14px; color:rgba(255,255,255,0.6);">日活 IP</div>
  </div>
  <!-- 第三个完全一样的盒子... -->
</div>
```
问题：三个等大盒子、字号均匀（24px vs 14px 只差 1.7 倍）、像 Bootstrap 卡片列表。

✅ **PPTX 思维**（极端字号反差、非对称重力、破界元素）：
```html
<!-- L1: 底层铺满卡片的迷你折线图作为视觉底纹 -->
<svg style="position:absolute; inset:0; width:100%; height:100%; opacity:0.08; z-index:1;">
  <polyline points="0,500 200,480 400,420 600,300 800,150 1000,80 1280,20"
    fill="none" stroke="var(--accent-1)" stroke-width="3"/>
</svg>
<!-- L4: 核心数字碾压式呈现 -->
<div style="position:absolute; top:100px; left:60px; z-index:4;">
  <div style="display:flex; align-items:baseline; gap:6px;">
    <span style="font-size:120px; font-weight:900; color:var(--accent-1);
      line-height:0.85; letter-spacing:-3px;">500万</span>
    <span style="font-size:13px; color:var(--text-secondary); opacity:0.6;">月访问量</span>
  </div>
  <div style="font-size:12px; color:var(--text-secondary); opacity:0.4; margin-top:4px;">
    从零起步仅 1 年即达此规模</div>
</div>
<!-- 辅助指标缩入右下角，极小极淡 -->
<div style="position:absolute; bottom:60px; right:60px; display:flex; gap:40px; z-index:3;">
  <div><span style="font-size:28px; font-weight:800; color:var(--accent-2);">30K+</span>
    <div style="font-size:10px; color:var(--text-secondary); opacity:0.5;">注册用户</div></div>
  <div><span style="font-size:28px; font-weight:800; color:var(--accent-4);">7K+</span>
    <div style="font-size:10px; color:var(--text-secondary); opacity:0.5;">日活 IP</div></div>
</div>
```
要点：120px vs 13px = **9.2 倍字号反差**；底层 sparkline 铺底（T10）；主数据独占左上 60% 画面；辅助数据退入右下暗角。
...
<div style="display:grid; grid-template-columns:1fr 1fr; gap:20px;">
  <div style="background:#1E1E1E; padding:24px;">方案 A 内容...</div>
  <div style="background:#1E1E1E; padding:24px;">方案 B 内容...</div>
</div>
```

✅ **PPTX 思维**（视觉降维碾压、非对称面积）：
```html
<!-- 左侧：被碾压的旧方案，蜷缩灰暗 -->
<div style="grid-column:1; background:var(--card-bg-to); padding:20px 16px;
  border-radius:var(--card-radius); opacity:0.7; border:1px solid var(--card-border);">
  <div style="font-size:14px; color:var(--text-secondary);">旧方案内容...</div>
</div>
<!-- 右侧：推荐方案，accent 灼焰 + 悬浮阴影 + 面积更大 -->
<div style="grid-column:2; background:linear-gradient(135deg, var(--accent-1), var(--accent-2));
  padding:28px 24px; border-radius:var(--card-radius);
  box-shadow: 0 8px 32px rgba(245,197,24,0.2); transform:translateY(-4px);
  clip-path: polygon(4% 0, 100% 0, 100% 100%, 0 100%);">
  <div style="font-size:16px; font-weight:700; color:var(--bg-primary);">推荐方案内容...</div>
</div>
<!-- 跨中线的 VS 标记，打破物理分隔 -->
<div style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%);
  width:48px; height:48px; border-radius:50%; background:var(--bg-primary);
  border:2px solid var(--accent-1); display:flex; align-items:center; justify-content:center;
  font-size:14px; font-weight:900; color:var(--accent-1); z-index:5;">VS</div>
```
要点：右侧用 `clip-path` 斜切制造"侵入感"；VS 标记跨中缝；左侧 `opacity:0.7` 压暗；右侧 `transform:translateY(-4px)` 浮起。

### 场景 3：标题区域

❌ **网页思维**：`<h1 style="font-size:28px; text-align:center; margin-bottom:20px;">标题</h1>`

✅ **PPTX 思维**（渐变文字 + 装饰线 + 偏心定位）：
```html
<div style="position:absolute; top:20px; left:40px; z-index:4;">
  <div style="font-size:11px; font-weight:700; letter-spacing:3px; color:var(--accent-1);
    opacity:0.6; margin-bottom:6px;">PART 02 · DATA STORM</div>
  <h1 style="font-size:28px; font-weight:700; line-height:1.2; margin:0;
    background:linear-gradient(135deg, var(--text-primary), var(--accent-1));
    -webkit-background-clip:text; -webkit-text-fill-color:transparent;">
    数据说话：增长有多快</h1>
  <div style="width:40px; height:3px; margin-top:8px; border-radius:2px;
    background:linear-gradient(90deg, var(--accent-1), var(--accent-2));"></div>
</div>
```
要点：overline 标签 + 渐变文字 + 下方短装饰线，三层构成标题系统。

### 场景 4：结构常规 vs 结构戏剧（V4 核心跃迁）

❌ **结构常规**（Grid 格子 + 装饰撒料 = V3 天花板）：
```html
<!-- 所有卡片都是矩形、都在 Grid 格子里、用装饰点缀来制造"精致感" -->
<div style="display:grid; grid-template-columns:2fr 1fr; gap:18px; padding:40px;">
  <div style="background:#1E1E1E; border-radius:12px; padding:24px; overflow:hidden;">
    <!-- 核心数字在卡片里 -->
    <span style="font-size:56px; color:#F5C518;">500万+</span>
    <p>月访问量</p>
  </div>
  <div style="background:#1E1E1E; border-radius:12px; padding:24px; overflow:hidden;">
    <!-- 辅助内容在另一个等矩形卡片里 -->
...
<!-- 撒上脉冲点、角标线、渐隐线来装饰 -->
```
问题：**空间结构是常规的**——卡片全矩形、全在格子里、数字被"关"在容器里。装饰再精致也改变不了底层的平庸。

✅ **结构戏剧**（空间本身就是设计语言 = V4 目标）：
```html
<!-- 核心数字脱离任何容器，裸露在空间中如同纪念碑 -->
<div style="position:absolute; top:80px; left:60px; z-index:4;">
  <span style="font-size:120px; font-weight:900; color:var(--accent-1);
    line-height:0.85; letter-spacing:-3px;
    text-shadow: 0 0 80px rgba(245,197,24,0.15);">500万+</span>
  <div style="font-size:13px; color:var(--text-secondary); opacity:0.5; margin-top:6px;">
    月访问量 · 从零起步仅 1 年</div>
</div>

<!-- 辅助内容用斜切卡片，底部被裁出动感 -->
<div style="position:absolute; top:280px; right:40px; width:480px;
  background:linear-gradient(135deg, var(--card-bg-from), var(--card-bg-to));
  border-radius:var(--card-radius); padding:24px; overflow:hidden;
  clip-path: polygon(0 0, 100% 0, 100% 100%, 6% 90%);">
  <ul>列表内容...</ul>
</div>

<!-- 底部一个出血色带冲出画布两侧 -->
<div style="position:absolute; bottom:80px; left:-40px; right:-40px; height:64px;
  background:linear-gradient(135deg, var(--accent-1), var(--accent-2));
  clip-path: polygon(2% 0, 100% 20%, 98% 100%, 0 80%);
  opacity:0.9; z-index:3;">
  <div style="padding:16px 80px; font-size:15px; font-weight:700; color:var(--bg-primary);">
    核心金句或结论...</div>
</div>

<!-- 装饰做减法：只有一个 500px 半透明几何色块取代 N 个小圆点 -->
<div style="position:absolute; top:-100px; right:-80px; width:500px; height:500px;
  border-radius:50%; background:radial-gradient(circle, var(--accent-1), transparent 70%);
  opacity:0.05; z-index:1;"></div>
```
要点：数字**脱框裸露**在空间中；辅助卡片用 `clip-path` 斜切底部；底部色带**出血冲出画布两侧**并斜切；装饰只有 1 个大几何形状取代 N 个小点缀。**空间结构本身就在叙事**。

### 场景 5：极端留白页 vs 均匀填充页

❌ **均匀填充**（每页都是 60-75% 填充率，翻页无惊喜）：
```html
<div style="padding:40px; display:grid; grid-template-columns:1fr 1fr; gap:20px;">
  <!-- 左右各一张卡片，填充率 65%，和其他页差不多 -->
</div>
```

✅ **极端留白**（90% 空白 + 一句话 = 翻页时的"窒息级"反差）：
```html
<!-- 整页只有这些元素，其余全是深黑虚空 -->
<div style="position:absolute; top:50%; left:15%; transform:translateY(-50%); z-index:4;
  max-width:500px;">
  <div style="font-size:11px; font-weight:700; letter-spacing:4px; color:var(--accent-1);
    opacity:0.5; margin-bottom:16px;">INSIGHT</div>
  <div style="font-size:32px; font-weight:700; line-height:1.5; color:var(--text-primary);">
    不是替代，而是补位 —— <span style="color:var(--accent-1);">Linux.do</span>
    填补了中文技术社区的生态空白</div>
</div>
<!-- 右下角极淡的 200px 水印 -->
<div style="position:absolute; bottom:-20px; right:-40px; font-size:200px; font-weight:900;
  color:var(--accent-1); opacity:0.025; z-index:1; pointer-events:none;">DO</div>
```
要点：填充率 < 30%；金句偏心于左侧 15% 而非居中；**翻页时从上一页的信息爆炸骤然切入虚空，制造认知重置**。

---

## ★★★ CSS 高级武器库（必须使用至少 2 种/页） ★★★

> 以下 8 种高级 CSS 技法带完整可复制代码。**每页至少使用 2 种**，否则你的 CSS 能力释放不足。这不是"可选装饰"，是 P1 级的设计基本功。
...
### 排版阶梯（严格分层 -- 字号反差是设计力的核心指标）

| 层级 | 用途 | 字号 | 字重 | 行高 | 颜色 | 强制要求 |
|------|------|------|------|------|------|---------|
| H0 | 封面主标题 | 48-160px | 900 | 0.85-1.1 | --text-primary | 封面标题**必须** >= 80px |
...
| Overline | PART 标识 | 10-12px | 700, letter-spacing: 2-4px | 1.0 | --accent-1 | 每个内容页**必须**有 overline |
| Data-Hero | **核心 KPI（每页的视觉锚点）** | **64-120px** | 900 | 0.85-1.0 | --accent-1 | **每个含数据的内容页必须有至少一个 64px+ 的数字** |
| Data-Sub | 辅助指标 | 28-40px | 800 | 1.0 | --accent-2/--accent-4 | 辅助数据不能与核心 KPI 同等大小 |

> **★ 字号反差硬性底线 ★**：每页最大字号与最小字号的**倍数比必须 >= 5 倍**。
> 例：核心数字 80px + 注解 12px = 6.7 倍 ✅ ；标题 28px + 正文 14px = 2 倍 ❌（太平庸）
> 如果你发现某页的最大字号/最小字号 < 5 倍，立即将核心元素字号调大或将辅助元素字号调小。
...
- `accent`：视觉爆裂点，通常一页 1 个效果最好（2 个也可以，但必须有明显的主次大小差异）
...
- 同一页渐变方向保持和谐（不要 3 个完全不同方向的渐变打架）
- 渐变的色彩来源应从 CSS 变量中取值（如 `linear-gradient(135deg, var(--accent-1), var(--accent-2))`）

### 色彩安全

- **硬底线**：正文文字与背景的对比度必须可读
- **硬底线**：accent 色作为文字颜色时，只用于标题/标签/数据数字，不用于大段正文
...
- **★ 核心数字必须脱框**：页面中最重要的 1 个 KPI 用 `position:absolute` 直接放在空间中（参考 CSS 武器库 9a），字号 80-120px，不放在任何卡片容器内。它独占画面 40-60% 的视觉注意力
- **构图策略**：脱框核心 KPI 占最大视觉锚点位（T2 极致字号），辅助指标在小卡片中以 28-40px 呈现，形成极端的主次反差
...
- **★ 极端留白**：填充率 < 30%，90% 画布为虚空。金句偏心放置（左侧 15% 或右侧 20%），**绝不居中**除非是刻意的"中心窒息"效果
...
### 硬底线（仅 5 条 -- 违反会导致跨页崩溃的底层约束）

| # | 硬底线 | 原因 |
|---|--------|------|
| 1 | **画布 1280x720px，body overflow:hidden** | body 裁切画布外的内容 |
| 2 | **font-family 统一** | 跨页字体一致 |
| 3 | **CSS 变量统一** | 风格一致的硬保证 |
| 4 | **卡片内容不溢出自身边界** | 每张卡片的**内部文字/图表**必须在卡片自身区域内完整可见（`overflow:hidden` + 正文 `-webkit-line-clamp` 截断）。但**卡片本身的位置和形状**完全自由——叠压、出血、斜切不受限制 |
| 5 | **禁止 CSS 动画** | PPTX 不支持 @keyframes / animation / transition |

> **硬底线之外的一切静态 CSS 手段完全自由。** text-align:center、justify-content:center、任意 border-radius 值、任意 padding 值——当设计意图需要时，都是合法的工具。
...
> **禁止**：`@keyframes`、`animation`、`transition`——PPTX 截图/SVG 转换时动画完全无效，是纯浪费 token。所有视觉效果必须是静态的。
...
以下不是"禁止"，而是"当你无意识地这样做时，停下来想想有没有更好的选择"：
...
4. **卡片全部兑现** -- 策划稿 `cards[]` 有几张，HTML 就必须有几张
5. **每张卡片有实质内容** -- text 有正文、data 有数字+可视化、list 有条目
6. **decoration_hints 已执行** -- 三个维度（background/card_accent/page_accent）都体现在 CSS 中
7. **卡片内容不溢出** -- 每张卡片的文字/图表在卡片自身区域内完整可见（overflow:hidden + line-clamp）。卡片本身的位置自由不受限
8. **无 CSS 动画** -- 不包含 `@keyframes`、`animation`、`transition`（PPTX 不支持）
