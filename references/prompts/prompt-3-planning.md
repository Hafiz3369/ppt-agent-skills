...
### ★★ 按 page_type 的策划深度指引（硬性）★★

> **核心原则**：品牌信息（演讲人/日期/公司）是页脚元素，不是卡片！不要把页脚信息包装成 cards[] 中的一个 card 来凑数。品牌信息由设计师自行放置在页面角落。
...
**封面策划三要素**（缺一不可）：
1. **一个视觉爆裂的数据/金句** -- 通过 data_highlight 或 quote 卡片承载，必须有 chart_type 实现可视化
2. **至少一个内容卡片** -- 承载主题关键词、社区基因、核心价值观、技术栈等有信息量的内容（tag_cloud / list / text），让设计师有素材可以构建画面层次
3. **品牌信息写入 content_summary，不要占用 cards[]** -- `supporting_points` 里写品牌/演讲人/日期，设计师从这里取出放到页面角落
...
**资源绑定必填（两层）**：每页 JSON 必须填写页面级 `required_resources`（layout/page_template/principles）和每个卡片的 `resource_ref`（block/chart/principle）。映射关系已在 4a 预读阶段获取（`resource-registry.md`），直接填写即可。下游 HTML 生成步骤将按此清单逐一读取资源，不再自行推断。
```
