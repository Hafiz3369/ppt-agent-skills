...
**禁止一次性输出所有页面的 JSON。每次调用只生成 1 页的策划 JSON。**

严格规则：
1. 每次调用只输出**一个** JSON 对象（一页的策划卡）
2. 封面页/目录页/章节封面/结束页等极简页可 2-3 页一起输出，但仍然每页独立文件
3. **禁止**用 Python/脚本/代码生成 planning JSON（策划稿是内容创作，不是脚本拼凑）
...
- 3 个并列 -> 三栏等宽 | **主次结合**（推荐）
- 4-6 个子项 -> **英雄式** | **混合网格**（推荐） | L 型 | T 型 | 瀑布流

#### 为每个区域选择 card_type（自由组合）

每页的卡片可自由组合 14 种 card_type，根据内容特征选择（完整选择指南见 `blocks/README.md`）：

**基础类型**（prompt-4 内联定义）：
`text` | `data` | `list` | `process` | `tag_cloud` | `data_highlight`

**复合类型**（按需从 `blocks/*.md` 加载）：
`timeline` | `diagram` | `quote` | `comparison` | `people` | `icon_group` | `image_hero` | `matrix_chart`

> 复合类型推荐使用 `grid-column: 1 / -1` 等跨行/跨列属性，与基础类型卡片在同一布局中共存。同一页不宜超过 1 个跨列跨行的复合组件。

### 第三步：为每页设计配图策略

> **策划阶段拥有最丰富的上下文**（主题/受众/搜索素材/每页内容/布局/卡片结构），因此配图决策必须在此阶段完成，而不是留给下游的 HTML 生成步骤自行决定。
...
> **打破"图片全当背景"的单调感**：整个 PPT 中，`split-content` 和 `card-inset` 应各至少使用 1 次（总页数 >= 8 时）。不要所有页都用 `hero-blend` 或 `atmosphere`。
...
- **光影氛围**：从风格表获取
- **质量锚定**：固定后缀，每个 prompt 必须追加

> **核心规则**：prompt 必须用英文，必须包含至少 3 个具象物体/场景细节。禁止模糊泛指如"business illustration"、"technology background"。尾部必须追加质量锚定后缀（见 `image-generation.md`）。
...
```json
{
  ...
  "cards": [
    {
      ...
      "card_type": "text | data | list | process | tag_cloud | data_highlight | timeline | diagram | quote | comparison | people | icon_group | image_hero | matrix_chart",
  ...
  "required_resources": {
    ...
    "blocks": ["blocks/{复合card_type}.md（cards[]中每个复合类型一条）"],
    "charts": ["charts/{chart_type}.md（cards[]中每个chart_type一条）"],
    "icons": ["icons/{分类}.md（需要图标时填）"],
    "principles": ["principles/{名称}.md（根据页面特征选 1-2 个）"]
  }
}
```
...
> **required_resources 填充规则**：
>
> 策划阶段已预读所有 README，知道完整的 ID -> 文件路径映射。**必须在此字段中列出下游 HTML 生成需要读取的全部资源路径**，让消费者无需再查映射表或凭记忆推断。
>
> | 字段 | 何时填 | 映射来源 |
> |------|-------|----------|
...
> | `blocks[]` | cards[] 中含复合 card_type 时 | card_type -> `resource-registry.md` 第 7 节 |
> | `charts[]` | cards[] 中含 chart_type 时 | chart_type -> `resource-registry.md` 第 3 节 |
> | `icons[]` | 卡片需要图标时 | 图标分类 -> `resource-registry.md` 第 4 节 |
> | `principles[]` | 每页至少 1 个 | 页面特征 -> SKILL.md 原则检索规则 |
>
> **填充示例**（一页包含英雄式布局 + timeline 复合组件 + KPI 图表）：
> ```json
> "required_resources": {
>   "layout": "layouts/hero-top.md",
>   "page_template": null,
>   "blocks": ["blocks/timeline.md"],
>   "charts": ["charts/kpi.md"],
>   "icons": ["icons/data-analytics.md"],
>   "principles": ["principles/data-visualization.md", "principles/visual-hierarchy.md"]
> }
> ```
>
> **填充示例**（封面页）：
...
- 零幻觉：所有数据必须来自搜索结果
- 覆盖所有页面（封面到结束页）
- 每页必须填写 `visual_weight`（参考 narrative-rhythm.md）
- 禁止空内容（每个卡片必须有实质内容）
- 720px 画布防溢出：内容量需适配画布高度

### 卡片数量与类型建议（非硬性）
- 推荐 3-5 张卡片，至少 2 种 card_type
- **每页至少 2 种 card_style**（硬性，打破"一堆同色方块"的单调感）
- accent 和 elevated 各最多 1 个/页
...
- **有数据时**推荐至少 1 张 data 卡片
- data/data_highlight 卡片必须指定 `chart_type`
...
### 其他约束
- 避免"单一焦点"布局，除非确实只需一个全屏内容
- 每页最多 1 个跨列跨行的复合组件（避免布局拥挤）
...
**required_resources 必填**：每页 JSON 必须包含 `required_resources` 字段，将该页使用的 layout_hint / card_type / chart_type / page_type 翻译为具体的资源文件路径。映射关系已在 4a 预读阶段获取（`resource-registry.md`），直接填写即可。下游 HTML 生成步骤将按此清单逐一读取资源，不再自行推断。
```
