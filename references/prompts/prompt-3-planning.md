...
> **布局多样性硬性约束（必须遵守）**：
>
> 1. **相邻 content 页禁止使用相同 layout_hint**（章节封面/目录等非 content 页不参与计算）
> 2. **全 PPT 范围内，任一 layout_hint 占比不得超过 30%**（如 10 页 content 页最多 3 页用同一布局）
...
>
> 策划阶段已预读所有 README，知道完整的 ID -> 文件路径映射。资源绑定必须精确到消费者（哪个卡片用哪个参考），而不是堆在页面公共清单里靠人推断。
>
...
> | 字段 | 何时填 | 映射来源 |
> |------|-------|----------|
> | `block` | 该卡片为复合 card_type 时 | card_type -> `resource-registry.md` 第 7 节 |
> | `chart` | 该卡片含 chart_type 时 | chart_type -> `resource-registry.md` 第 3 节 |
> | `icon` | 该卡片需要图标时 | 图标分类 -> `resource-registry.md` 第 4 节 |
...
> ```
>
> > **基础类型卡片**（text/list/process/tag_cloud/data_highlight）如果不需要图表/图标/特殊原则，`resource_ref` 填空对象 `{}`。
>
...
### 布局多样性（硬性）
- **相邻 content 页禁止相同 layout_hint**
- **任一 layout_hint** 在全 PPT 占比不超过 30%
...
**资源绑定必填（两层）**：每页 JSON 必须填写页面级 `required_resources`（layout/page_template/principles）和每个卡片的 `resource_ref`（block/chart/icon/principle）。映射关系已在 4a 预读阶段获取（`resource-registry.md`），直接填写即可。下游 HTML 生成步骤将按此清单逐一读取资源，不再自行推断。
```
