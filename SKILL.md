...
**通用生命周期**：`create(--model MAIN_MODEL) → RUN(prompt路径) → STATUS… → FINALIZE → close`；完成即关，不复用。创建时**必须**显式传 `--model MAIN_MODEL`，禁止省略。
...
1. **回查** Section 3.1.1 输出的《Subagent 操作手册》，取出其中的**调用模板**
2. **变量替换**：将模板中的 `{{SUBAGENT_NAME}}`、`{{PROMPT_PATH}}`、`{{MODEL}}` 替换为当前步骤的实际值
...
#### 3.1.0 模型感知与防降格（Model Perception）
为了绝对保证内容质量不滑坡，主 agent 必须在开局时确认自己是谁：
1. **强行识别当前主 agent 正在使用的大模型版本**（例如 Claude-3.5、Gemini-1.5 等，如果无法确认直接问用户）。
2. 将其在心中显性固化为 `MAIN_MODEL` 全局变量，并在对话中输出 `## 模型感知结果`。
3. **全局防降格红线**：一旦确认 `MAIN_MODEL`，在后续流程中创建任何 Subagent 时，必须强制携带该模型参数发起子系统（绝对禁止走默认回退配置）。
...
3. 整理出支持情况并输出到对话，标题固定为 `## Subagent 操作手册`，必须包含以下内容：
   ...
   - **调用模板（必须含变量槽）**：一个可参数化的命令模板，包含 `{{SUBAGENT_NAME}}`、`{{PROMPT_PATH}}`、`{{MODEL}}` 三个槽位
   - **示例调用**：用具体值填充槽位的实例
...
- **必须覆盖但允许精简（如果已知）的维度**：场景、受众、目标动作、期望页数与密度、风格倾向、品牌规范、必含内容、避讳、语言、配图策略（要不要 AI 画图）、资料使用范围。
...
| 细分阶段 | 操作 | 产物目标 |
|---------|------|----------|
| **4A 页面规划** | 按《自适应调用协议》显式组装命令并创建 PageAgent-N（新建 session，`--model MAIN_MODEL`） | `planningN.json` |
