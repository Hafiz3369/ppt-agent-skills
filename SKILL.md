...
**通用生命周期**：`create(--model SUBAGENT_MODEL) → RUN(prompt路径) → STATUS… → FINALIZE → close`；完成即关，不复用。**唯一例外**：Step 4 的 Codex 模式允许同一 `PageAgent-N` 在单页内部按 `4A → 4B → 4C` 续写，直到整页 FINALIZE 后立刻关闭。创建时**必须**显式传 `--model SUBAGENT_MODEL`，禁止省略。`SUBAGENT_MODEL` 由用户在 Step 0 采访时指定（详见 3.1.0 及 6.2）。
...
| P4 | PageAgent-N（每页一个） | 页面规划 + HTML + 审查 | planningN.json, slide-N.html, slide-N.png | 同上，按模式（Codex/Claude）编排三阶段 |
...
- **Step 0 默认强制模板化**：主 agent 必须先通过 `prompt_harness.py` + `references/prompts/tpl-interview.md` 生成 `OUTPUT_DIR/runtime/prompt-interview.md`，再依据渲染结果向用户发问；不得把采访退化成随手写的一小段简陋问题。
...
3. 整理出支持情况并输出到对话，标题固定为 `## Subagent 操作手册`，必须包含以下内容：
   ...
   - **续写模板（若支持）**：用于 Step 4 4B/4C，至少包含 `{{SESSION_HANDLE}}` 与 `{{PROMPT_PATH}}` 两个槽位
   - **示例调用**：用具体值填充槽位的实例
   - **支持的执行模式**：是否支持 session 续写（决定 Step 4 走 Codex 还是 Claude 模式）

   调用模板示例（主 agent 必须根据实际环境生成类似格式，`{{MODEL}}` = `SUBAGENT_MODEL`）：
   ```
   # 模板（槽位用 {{}} 标记，MODEL 取自用户在采访阶段指定的 SUBAGENT_MODEL）
   codex --model {{MODEL}} --message "Read {{PROMPT_PATH}} and execute all instructions" --name {{SUBAGENT_NAME}}
   # 或
   claude --model {{MODEL}} --message "Read {{PROMPT_PATH}} and execute all instructions" --name {{SUBAGENT_NAME}}
   # 若支持续写，再额外给出 resume 模板
   codex resume {{SESSION_HANDLE}} --message "Read {{PROMPT_PATH}} and continue"
   ```
...
4. Step 0 发问前，必须先回查这一结论；支持则优先结构化采访 UI，不支持再文本回退。
...
| `ROOT_OUTPUT_DIR` | `CWD/ppt-output/`（CWD = 主 agent 当前工作目录，禁止跳出） |
...
- **默认执行方式**：先生成 `OUTPUT_DIR/runtime/prompt-interview.md`，再按该模板渲染结果组织对外提问；若环境支持结构化采访 UI，则优先让 CLI 执行结构化采访，不允许跳过这一层直接退回纯文本问答。
- **必须覆盖但允许精简（如果已知）的维度**：场景、受众、目标动作、期望页数与密度、风格倾向、品牌规范、必含内容、避讳、语言、配图策略（要不要 AI 画图）、资料使用范围、**subagent 模型选择**。
- **subagent 模型选择（必问）**：必须向用户确认「后续所有子代理使用什么模型？」。列出当前环境可用的模型选项供用户选择（如 `claude-sonnet-4-20250514`、`o4-mini`、`gemini-2.5-pro` 等，根据实际环境探测结果列出）。用户选定后固化为 `SUBAGENT_MODEL` 全局变量（详见 3.1.0）。若用户表示「跟主 agent 一样」或不关心，则 `SUBAGENT_MODEL = MAIN_MODEL`。
- **失败回退**：若结构化采访 UI 不可用，则回退为文本问答；只有在 Step 0 的模板生成命令失败且确认为 `BLOCKED_SCRIPT_INTERFACE` 时，才允许绕过 `prompt-interview.md` 直接发问；并在后续 `interview-qa.txt` 中保留这一异常说明。
...
| 细分阶段 | 操作 | 产物目标 |
|---------|------|----------|
| **4A 页面规划** | 按《自适应调用协议》显式组装命令并创建 PageAgent-N（新建 session，`--model SUBAGENT_MODEL`） | `planningN.json` |
...
- **子代理内部自检**替代主 agent 细粒度阶段间 Gate；主 agent 仅在回收 FINALIZE 后做**整页终检**（`planning_validator` + 文件存在性 + FINALIZE 状态核对）
